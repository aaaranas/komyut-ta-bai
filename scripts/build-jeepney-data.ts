...
async function main() {
  ...
  // Build routes from geocoded landmarks (drop misses, then dedupe again).
  const generatedRoutes: object[] = [];
  const dropped: string[] = [];
  for (const route of parsed) {
    const ids = route.landmarks
    ...
    const coords = ids.map((id) => {
      const stop =
        generatedStops.find((s) => s.id === id) ??
        { lat: 0, lng: 0 }; // curated stops resolved below
      return stop;
    });
    // Resolve curated stops' coordinates for distance estimation.
    ...
    for (let i = 0; i < ids.length - 1; i++) {
      const a = curated[ids[i]] ?? coords[i];
      const b = curated[ids[i + 1]] ?? coords[i + 1];
      totalKm += haversineKm(a.lat, a.lng, b.lat, b.lng);
    }
    const source = experimentalJeepneyRoutes.find(
      (r) => r.code === route.code
    )!;
    const firstName = titleCase(route.landmarks[0]);
    const lastName = titleCase(route.landmarks[route.landmarks.length - 1]);
  ...
  writeFileSync(
    ...
    banner("Stops") +
      ...
      `export const jeepneyStops: Stop[] = ${JSON.stringify(generatedStops, null, 2)};\n`
  );
  writeFileSync(
  ...
  console.log(
    `Wrote ${generatedStops.length} stops, ${generatedRoutes.length} routes`
  );
  if (dropped.length > 0) {
